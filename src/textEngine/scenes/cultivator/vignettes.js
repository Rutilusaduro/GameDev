...
function reneeSubject(testerName) {
  return { id: 10, name: 'Reneé', first: 'Reneé' };
}

const RENEE_HARVEST_KEY = { 5: 'heavy', 6: 'fat', 7: 'veryFat', 8: 'enormous', 9: 'colossal', 10: 'colossal' };
const RENEE_DIGEST_KEY = { 5: 'heavy', 6: 'fat', 7: 'veryFat', 8: 'enormous', 9: 'colossal', 10: 'blob' };

function reneeHarvestKey(stageId) {
  return RENEE_HARVEST_KEY[stageId] ?? 'heavy';
}

function reneeDigestKey(stageId) {
  return RENEE_DIGEST_KEY[stageId] ?? 'heavy';
}
...
const HARVEST_VIGNETTES_PLANNED = {
  heavy: {
  ...
  enormous: {
    ...
    10: (name) => `The immense ${name} dominates the room when she arrives. Reneé completes the final harvest exactly on schedule. She absorbs the whole yield, swelling vastly larger herself. Immobile and content, Reneé processes the success in the reorganized apartment.`,
  },
  colossal: {
    6: (name) => `${name} arrives at the apartment now arranged entirely around Reneé's immense form. Reneé greets her with clinical precision. The harvest proceeds as planned — she consumes the fattened tester, adding the mass to her already vast body. Reneé logs the data mentally at first, satisfied with how the ritual closed out.`,
...
const HARVEST_VIGNETTES_EMERGENCY = {
  heavy: {
  ...
  fat: {
    6: (name) => `Halfway through the tasting ${name} becomes alarmed and dashes for the door. Reneé, wider with a rolling gait, chases her down and pins the tester against the counter. She consumes the fattened tester with hurried precision, irritation evident in her posture. The kitchen shows small signs of disruption. Reneé sits wider afterward, adjusting her clothes as she logs the yield with dissatisfaction at the forced timing.`,
  ...
  enormous: {
    6: (name) => `Suspicion hits ${name} during the ongoing tasting and she bolts for the exit. Reneé, enormous and filling the couch, uses her reach and weight to block and drag the tester back. She pins her and devours the fattened tester with focused determination. Small messes spread across the floor. Reneé logs the yield afterward, barely shifting her vast frame, irritated at the imperfect timing.`,
    ...
    10: (name) => `The immense ${name} panics during the session but cannot move far. Reneé, enormous herself, simply closes in and devours the entire mountain of cultivated flesh where the tester sits. She absorbs the whole yield, swelling vastly larger. Immobile and content beneath her irritation, Reneé processes the emergency success in the reorganized apartment.`,
  },
  colossal: {
    6: (name) => `Mid-tasting ${name} grows suspicious and tries to slip away. Reneé, colossal and nearly immobile, blocks the doorway with her immense girth and pulls the tester back. She consumes the fattened tester with precise, if rushed, bites. The added mass integrates into her vast body. Reneé logs the data mentally at first, satisfied with the completion despite the emergency.`,
...
const RECRUITMENT_SCENE = `Reneé reads appetite the way some people read tells. The ones who come back without being invited. The ones who finish everything and study the empty dish, wondering if asking for more is allowed. The ones who say "I probably shouldn't" and then do. She has a short list of names. She's ready to choose.`;



const DIGEST_VIGNETTES = {
  heavy: {
  ...
  fat: {
    ...
    latePlus3: (name) => `After weeks absorbing the enormous yield of ${name}, Reneé rests content. Her vast ass fills the reinforced seating completely, floorboards worn from her weight. The cycle is closed and she knows it.`,
  },
  veryFat: {
    ...
    earlyPlus3: (name) => `In the time after fully eating the immense ${name}, Reneé dominates the room. Her colossal ass and thickened limbs make every shift slow and total, furniture protesting constantly. She remains very still in the warm kitchen, deep inside the metabolic completion.`,
    ...
    latePlus3: (name) => `After extended processing of the yield from devouring ${name}, Reneé sits as a soft, warm mountain. Her immense ass fills the space, floor reinforced beneath her. Calm. Complete. Hungry again already.`,
  },
  enormous: {
    ...
    earlyPlus3: (name) => `Following consumption of the massive ${name}, Reneé becomes an even greater presence. Her colossal ass and belly dominate the room, cracking a nearby tile on one shift. She focuses inward on the processing with calm professionalism.`,
  ...
  colossal: {
    ...
    earlyPlus3: (name) => `Following the enormous feast on ${name}, Reneé is a mountain. Her immense ass dominates the reorganized room, every subtle shift testing the reinforced foundations while she remains profoundly still.`,
    ...
    latePlus2: (name) => `Reneé's body has settled after extended integration of ${name}. Her enormous frame fills the apartment as intended, floor and furniture fully adapted. She is satisfied and ready for what comes next.`,
  },
  blob: {
    ...
    lateSpecial: (name) => `Over the long weeks processing the gluttony of devouring ${name}, Reneé remains an enormous, warm centerpiece. Her vast ass and cascading softness have fully integrated, the floor and supports now permanently strengthened beneath her. She regards visitors with calm, clinical eyes, satisfied and complete. The kitchen functions quietly around her permanent presence.`,
  },
};
...
const BLOB_SPECIAL = `Reneé becomes something more. The final harvest swells her into true immobility, her body expanding to fill a significant portion of the living space. Her vast, warm belly spreads across her lap and beyond, rolls cascading everywhere, limbs almost lost in the soft mountain of flesh. The room has been completely rearranged — furniture moved, pathways eliminated — to accommodate her enormous presence. She breathes slowly, satisfied, feeling every new inch of herself as the perfect outcome of the cycles. When you visit she regards you with calm, clinical eyes from within her enormous softness. She is exactly where she belongs, processing and complete. The kitchen still functions around her. Everything is as intended.`;

const GROWTH_REACTIONS = {
    5: {
    ...
    6: {
      ...
      3: `The enormous addition keeps Reneé in place. She feels her body swell in real time: belly cascading more toward her knees, hips widening dramatically, every limb growing chunkier and heavier. The reinforced furniture groans softly as she becomes a much larger presence in the room. You find her sitting very still in the warm kitchen, now very fat and deeply engaged in processing. Her breathing is slow and satisfied. She looks at you with calm eyes, entirely within the completion of the cycle. The result pleases her.`,
    },
    7: {
...
function harvestPoolKey(kind, reneeStageId, testerStageId) {
  ...
  return `cultivator.harvest.${kind}.${rk}.t${testerStageId}`;
}
...
export function renderCultivatorRecruitment(week = 1) {
  const ctx = createContext({ subject: reneeSubject(), week, globals: { featureId: 'cultivator' } });
  return render('{cultivator.recruitment}', ctx)?.trim() || RECRUITMENT_SCENE;
}

export function renderCultivatorDigest(reneeStageId, testerName, stagesJumped, isLate, week = 1) {
  const rk = reneeDigestKey(reneeStageId);
  if (rk === 'blob') {
    const key = `cultivator.digest.blob.${isLate ? 'late' : 'early'}`;
    return renderPool(key, testerName, week, { stagesJumped });
  }
  const j = Math.max(1, stagesJumped || 1);
  const prefix = isLate ? 'latePlus' : 'earlyPlus';
  for (let attempt = j; attempt >= 1; attempt--) {
    const key = `cultivator.digest.${rk}.${prefix}${attempt}`;
    const line = renderPool(key, testerName, week, { stagesJumped: attempt });
    if (line) return line;
  }
  return '';
}

export function renderCultivatorGrowth(beforeStageId, gain, stagesJumped, week = 1) {
  const j = Math.max(1, stagesJumped || 1);
  const afterStageId = Math.min(10, beforeStageId + j);
  if (afterStageId >= 10 && beforeStageId <= 8) {
    return renderPool('cultivator.growth.blob', 'Reneé', week) || BLOB_SPECIAL;
  }
  const row = GROWTH_REACTIONS[Math.min(beforeStageId, 9)] || GROWTH_REACTIONS[5];
  const maxJ = Math.max(...Object.keys(row).map(Number));
  const pick = Math.min(j, maxJ);
  const key = `cultivator.growth.s${Math.min(beforeStageId, 9)}.j${pick}`;
  return renderPool(key, 'Reneé', week, { startStage: beforeStageId, endStage: afterStageId, stagesJumped: j, gainLbs: gain }) || row[pick] || row[1] || '';
}
