...
export function renderSessionRaeArrival(stageIdx, student, week) {
  ...
  const raw = render(`{session.rae.arrival.s${si}}`, ctx)?.trim() || row?.arrival || 'Delivery.';
  const extra = row?.extra
    ? (render(`{session.rae.extra.s${si}}`, ctx)?.trim() || row.extra)
    : '';
  const combined = [raw, extra].filter(Boolean).join(' ');
  return appendV2Depth(combined, 'rankedSession', ctx, 0.24);
}
...
export function renderSessionRaeExtra(stageIdx, student, week) {
  ...
  return appendV2Depth(raw, 'rankedSession', ctx, 0.26);
}
...
export function renderSessionPayoff(stageIdx, gain, endReason, student, week) {
  ...
  return appendV2Depth(raw, 'rankedSession', ctx, 0.3);
}
