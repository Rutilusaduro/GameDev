...
import { RA_RANKS, INFLUENCE_PAIRS, NARRATIVE_EVENTS } from './gameData/content.js';
...
import { LILITH_ID, HUNT_NODES, HUNT_MEN, physicalMovesForOwned, drawReplies, getGuyLine, seduceSuccessChance, WILLPOWER_START, MAX_APPREHENSION, getEffectiveDifficulty, getConsumeText, DELIVERY_SCENE, CLUE_FEAST_LINE, LILITH_PASSIVE_GAIN, startLilithDigest, tickLilithDigest, canStartLilithHunt } from './gameData/lilith.js';
...
import { renderHuntNode, renderHuntTarget, renderLilithFeast, renderLilithDeliveryIntro, renderHuntTravel, renderHuntDormOpen, renderHuntClueFeast, renderLilithDigestBlocked, renderLilithDigestComplete } from './textEngine/scenes/hunt/index.js';
import { renderStageDrop } from './textEngine/scenes/overhaul/stageDrop.js';
...
import {
  applyCultDistribution, tickCultWeek, pickInterruptCompound, cultRoutesForOwned,
...
import { renderCircuitBeat, renderCircuitOpen } from './textEngine/scenes/floorCircuit/index.js';
import {
...
export default function HallPass(){
  ...
  useEffect(() => {
    const prime = () => warmupHallPassAudio();
  ...
  const [campusState,setCampusState]=useState({
    ...
    log:[renderCampusArrive(CAMPUS_CONFIG.startNode, 1)],
  ...
  const applyGainToStudent=(s,gain)=>{
    ...
    INFLUENCE_PAIRS.forEach(([a,b])=>{
      if((s.id===a||s.id===b)){
        ...
        if(other && getStage(other.lbs).id>=3) bonusInfluence+=Math.floor(gain*(0.15+(ownedHallSkills?.group_dynamics?0.08:0)));
      }
    });
  ...
  const getCampusExplorationCtx=()=>buildExplorationContext({
    ...
    floorYield:aggregateHallLoungeSkillEffects(ownedHallSkills||{}).campusYieldBonus||0,
  });
  ...
  const toggleFloorCircuitPin=(roomId)=>{
    setFloorCircuit((cur)=>toggleCircuitPin(cur||createInitialFloorCircuit(),roomId));
  };

  const applyFloorStudentPatches=(patches,prevStudents)=>{
    let next=prevStudents;
    const feedNow=(s,cal,full,rel,label)=>{
      const fed=feedStudentCalories(s,cal,full,rel,label,{ignoreDiscontent:true});
      return fed||s;
    };
    for(const p of patches||[]){
      let s=next.find((x)=>x.id===p.id);
      if(!s) continue;
      if((p.cal||0)>0||(p.full||0)>0){
        s=feedNow(s,p.cal||0,p.full||0,p.rel||0,'After-Hours leftover');
      }else if(p.rel){
        s={...s,relationship:Math.min(100,(s.relationship||0)+p.rel)};
      }
      if(p.corruption) s={...s,corruption:addCorruption(s,p.corruption)};
      if(p.lbs) s=processStudentGain(s,p.lbs,0);
      if(p.mood) s={...s,mood:p.mood};
      if(p.hunger) s=adjustHunger(s,p.hunger);
      next=next.map((x)=>x.id===s.id?s:x);
    }
    return next;
  };

  const walkFloorCircuit=()=>{
    const circuit=floorCircuit||createInitialFloorCircuit();
    const result=walkAfterHours({owned:ownedHallSkills||{},students,circuit,week,ap,rng:Math.random});
    if(!result.ok){push(`⚠️ ${result.reason}`);return;}
    if(result.cost) setAp((a)=>a-result.cost);
    const opener=renderCircuitOpen(week,{subject:students.find((s)=>!s.hidden)||students[0]});
    if(opener) push(`🌙 ${opener}`);
    result.beats.forEach((beat)=>{
      const subj=beat.studentId!=null?students.find((s)=>s.id===beat.studentId):students.find((s)=>!s.hidden);
      const prose=renderCircuitBeat(subj,week,beat.roomId);
      push(prose||result.logLines.find((l)=>l.includes(beat.label))||`🌙 ${beat.text}`);
    });
    setStudents((prev)=>applyFloorStudentPatches(result.studentPatches,prev));
    if(result.scrutinyDelta){
      if(result.scrutinyDelta<0) setAdminScrutiny((prev)=>Math.max(0,prev+result.scrutinyDelta));
      else addScrutiny(result.scrutinyDelta);
    }
    if(result.pantryDrops>0){
      setInventory((prev)=>{
        const next={...prev};
        for(let i=0;i<result.pantryDrops;i+=1){
          const item=rollWeeklyItem();
          next[item.id]=Math.min(INVENTORY_CONFIG.maxStack,(next[item.id]||0)+1);
        }
        return next;
      });
      push('🎒 After-Hours storage: extra pantry drop.');
    }
    setFloorCircuit(result.nextCircuit);
  };
  ...
  const rollCampusEvent=(nodeId,isTravel=true)=>{
    ...
    if(Math.random()<CAMPUS_CONFIG.itemFindChance*0.5+(aggregateHallLoungeSkillEffects(ownedHallSkills||{}).campusYieldBonus||0)*0.08){
      const item=rollWeeklyItem();
      setInventory(prev=>({...prev,[item.id]:Math.min(INVENTORY_CONFIG.maxStack,(prev[item.id]||0)+1)}));
  ...
  const moveToCampusNode=(nodeId)=>{
    ...
    const arrive=renderCampusArrive(nodeId,week);
  ...
  const executeItemFeed=(item,studentId,compoundId,modeId='feed')=>{
    const target=students.find(st=>st.id===studentId);
    if(!target) return;
    const compoundLabel=compoundId?COMPOUNDS[compoundId]?.label:null;
    const extraCals=itemCalorieBonus(item.label,ownedHallSkills||{});
    const mode=itemUseModesForOwned(ownedHallSkills||{}).find(m=>m.id===modeId)||{calMult:1,fullMult:1,extraRel:0};
    const fed=feedStudentCalories(target,Math.round((item.cal+extraCals)*mode.calMult),Math.round(item.full*mode.fullMult),1+(mode.extraRel||0),
  ...
  const useItemOn=(item,studentId,modeId='feed')=>{
    ...
    if(compounds.length>0){
      setCompoundFeedPicker({kind:'item',item,studentId,modeId});
      return;
    }
    executeItemFeed(item,studentId,null,modeId);
  };
  ...
  const advanceWeek=()=>{
    ...
    let updated=students.map(s=>{
      ...
      if(s.id===LILITH_ID&&lilithUnlocked){
        let next=processStudentGain(s,LILITH_PASSIVE_GAIN,0);
        const t=tickLilithDigest(s.lilithDigest);
        if(t.complete&&t.lbs>0){
          next=processStudentGain({...next,lilithDigest:t.digest},t.lbs,0);
          setTimeout(()=>push(`🌑 ${renderLilithDigestComplete(next,newWeek)}`),90);
          return next;
        }
    ...
    if(planPay.studentPatches.length){
      updated=applyFloorStudentPatches(planPay.studentPatches,updated);
    }
    if(planPay.moneyDelta) setMoney((m)=>Math.max(0,m+planPay.moneyDelta));
    ...
    // ── PANTRY RESTOCK ──
    {
      const drops=rnd(INVENTORY_CONFIG.weeklyDrops[0],INVENTORY_CONFIG.weeklyDrops[1])+(loungeSkillFx.pantryBonus||0);
      const found=[];
      setInventory(prev=>{
        const next={...prev};
    ...
    {
      const ritual=midnightRitualTick({owned:ownedHallSkills||{},students:updated,rng:Math.random});
      if(ritual.ok){
        updated=applyFloorStudentPatches(ritual.studentPatches,updated);
        ritual.logLines.forEach((line,i)=>setTimeout(()=>push(line),90+i*40));
      }
    }
    ...
    let exposedCount=0;
    updated=updated.map(s=>{
      let disc=Math.max(0,(s.discontent||0)-DISCONTENT_WEEKLY_DECAY-comfortFramingDecay(ownedHallSkills||{}));
  ...
  const doEvolvedActivity=(s)=>{
    ...
    const modular=renderEvolvedActivity(s,week,{formId:s.evolvedForm,stageIdx});
    if(modular) text=modular;
    ...
    setEvolvedActivityModal({student:s,stageIdx,text,floorFollowups:true});
  };

  const applyActivityFollowup=(id)=>{
    const modal=evolvedActivityModal;
    if(!modal?.floorFollowups||modal.followupUsed) return;
    const extra=extraActivityFollowups(ownedHallSkills||{}).find(e=>e.id===id);
    if(!extra) return;
  ...
  const makeHomeroomActivityChoice=(choiceId)=>{
    ...
    if(type==='conference'){
      ...
      const choices=homeroomChoicesForPhase(actDef,type,phaseIdx,ownedHallSkills||{});
      choice=choices.find(c=>c.id===choiceId); if(!choice) return;
  ...
  const completeFeast=()=>{
    ...
    if(tiffany){
      ...
      setStudents(prev=>prev.map(s=>s.id===tiffany.id?{...s,lbs:s.lbs+tiffanyLbs,relationship:Math.min(100,s.relationship+feastRelTotal)}:s));
      push(`✦ ${tiffany.name} — Wednesday Feast: +${tiffanyLbs} lbs · +${feastRelTotal} rel`);
    }
  ...
  const openLilithHunt=()=>{
    ...
    if(!canStartLilithHunt(lilith.lilithDigest)){
      push(`🌑 ${renderLilithDigestBlocked(lilith, week)||'Lilith is still processing. Hunt later.'}`);
      return;
    }
    ...
    setLilithHuntState({textLog:[{text:"HER DORM · ROOM 312",type:'location'},{text:renderHuntDormOpen(lilith,week),type:'narrative'}],currentNode:'dorm',encounter:null,deliveryMode:false,deliveryDone:false,aibTarget:null});
  };
  const openLilithAibHunt=(memberId)=>{
    ...
    setLilithHuntState({
      textLog:[
        ...
        {text:renderHuntDormOpen(lilith,week),type:'narrative'},
  ...
  const navigateHunt=(nodeId)=>{
    ...
    const travelText=(lilith&&renderHuntTravel(fromNode,nodeId,lilith,week))||null;
    const entries=[];
    if(travelText) entries.push({text:travelText,type:'action'});
    entries.push({text:node.label.toUpperCase(),type:'location'});
    const nodeDesc=lilith?renderHuntNode(nodeId,lilith,week):'';
    entries.push({text:nodeDesc,type:'narrative'});
  ...
  const approachMan=(manId)=>{
    ...
    const desc=renderHuntTarget(man.id,lilith,week);
    const entries=[
  ...
  const makeSeduction=(moveId)=>{
    if(!lilithHuntState?.encounter) return;
    const lilith=students.find(s=>s.id===LILITH_ID); if(!lilith) return;
    const move=physicalMovesForOwned(ownedHallSkills||{})[moveId]; if(!move) return;
    const{encounter}=lilithHuntState;
    if(encounter.won||encounter.failed||encounter.consumed) return;
    const stageId=getStage(lilith.lbs).id;
  ...
  const consumeMan=()=>{
    ...
    const extraHunt=kitchenHuntBonus(gain,ownedHallSkills||{});
    const totalGain=gain+extraHunt;
    setStudents(prev=>prev.map(s=>s.id===LILITH_ID?{...s,lbs:s.lbs+totalGain,lilithDigest:startLilithDigest(stageId)}:s));
    setLilithKillCount(k=>k+1);
    if(isAib){
      setOpposition(prev=>removeConsumedAibMember(prev,aibTarget.aibMemberId));
      addScrutiny(-15);
      push(`🩸 Lilith devours ${aibTarget.name} — board member removed (+${totalGain} lbs, −15 scrutiny)`);
      const consumeText=renderLilithFeast(lilith, stageId, week);
      setLilithHuntState(prev=>({
        ...prev,
        aibTarget:null,
        encounter:{...prev.encounter,consumed:true},
        textLog:[...prev.textLog,{text:consumeText,type:'narrative'},{text:`✦ +${totalGain} lbs · ${aibTarget.name} consumed`,type:'system'}],
      }));
      setOpposition(prev=>({
        ...prev,
    ...
    push(`🌑 Lilith — hunt complete: +${totalGain} lbs`);
    const consumeText=renderLilithFeast(lilith, stageId, week);
    setLilithHuntState(prev=>({...prev,encounter:{...prev.encounter,consumed:true},textLog:[...prev.textLog,{text:consumeText,type:'narrative'},{text:`✦ +${totalGain} lbs`,type:'system'}]}));
  };
  const deliveryScene=()=>{
    const lilith=students.find(s=>s.id===LILITH_ID); if(!lilith) return;
    const stageId=getStage(lilith.lbs).id;
    ...
    setStudents(prev=>prev.map(s=>s.id===LILITH_ID?{...s,lbs:s.lbs+totalGain,lilithDigest:startLilithDigest(stageId)}:s));
    setLilithKillCount(k=>k+1);
    push(`🌑 Lilith — delivery: +${totalGain} lbs`);
    const feastText=renderLilithFeast(lilith, stageId, week);
    setLilithHuntState(prev=>({...prev,deliveryDone:true,textLog:[...prev.textLog,{text:feastText,type:'narrative'},{text:`✦ +${totalGain} lbs`,type:'system'}]}));
  };
  ...
  const runArrivalCapstone=(s)=>{
    ...
    setEvolvedActivityModal({ student:s, stageIdx:getEvolvedActivityStageIdx(s), text:renderEvolvedActivity(s, week) || capstone.label });
  };
  ...
  const runImmobilityArrival=(s)=>{
    ...
    setEvolvedActivityModal({ student:s, stageIdx:getEvolvedActivityStageIdx(s), text:fullProse });
  };
  ...
  const runImmobilityRefit=(s)=>{
    ...
    setEvolvedActivityModal({ student:s, stageIdx:getEvolvedActivityStageIdx(s), text:prose });
  };
  ...
  const confirmLabSession=(sessionToSave)=>{
    ...
    if(ap<act.apCost){ push(`⚠️ Need ${act.apCost} AP.`); return; }
    setAp(a=>a-act.apCost);
    const extra=extraLabKitchenLbs(ownedHallSkills||{});
    const gain=rnd(...act.taliaGain)+extra;
  ...
  const selectCultRoute=(routeId)=>{
    ...
    const route=cultRoutesForOwned(ownedHallSkills||{}).find(r=>r.id===routeId);
    if(!route||ap<route.apCost){push(`⚠️ Need ${route?.apCost||1} AP.`);return;}
    setAp(a=>a-route.apCost);
    const { state: nextPs, outcome }=applyCultDistribution(pharmacistState,routeId,rnd,ownedHallSkills||{});
    if(!outcome) return;
  ...
  const startCommunityResearcherPanelReview=(s)=>{
    ...
    const stageIdx=getEvolvedActivityStageIdx(s);
    const evDef=EVOLVED_EVENTS['community_researcher']?.[stageIdx];
    if(!evDef){push('⚠️ No athletics panel review scheduled at this weight.');return;}
    setAp(a=>a-meta.apCost);
    setEvolvedEventState({studentId:s.id,formId:'community_researcher',stageIdx,phaseIdx:0,history:[],logLines:[],gainAccum:0,relAccum:0,done:false,endingText:null,gainBonus:0,relBonus:0});
  };
  ...
  const completeCaseStudy=(s)=>{
    ...
    setAp(a=>a-1);
    const extra=extraCaseStudyLbs(ownedHallSkills||{});
    const gain=rnd(gMin,gMax)+extra;
  ...
  const makeHaveAChatChoice=(choiceId)=>{
    ...
    const extra=extraHaveAChatChoices(crs.chatMemberIdx,crs.chatPhaseIdx,ownedHallSkills||{}).find(c=>c.id===choiceId);
    const choice=extra||phase?.choices?.find(c=>c.id===choiceId);
    if(!choice) return;
    const recorded=choice.winFlag||choiceId;
    const newHist=[...crs.chatHistory,recorded];
  ...
  const doContestAction=(action)=>{
    if(!eatingContestState) return;
    const{stageIdx,yourFoods,mayaFoods,yourFullness,mayaFullness,maxYourFullness,maxMayaFullness,yourGain,mayaGain,pantsFactor,actions,studentId}=eatingContestState;
    const s=students.find(st=>st.id===studentId);
    let updates={};
    let popup='';
    const extra=extraContestActions(ownedHallSkills||{}).find(a=>a.id===action);
    if(action==='unbutton'){
      ...
      updates.actions={...actions,[action]:true};
      if(extra.pantsDelta) updates.pantsFactor=pantsFactor+extra.pantsDelta;
      if(extra.fullnessDelta) updates.yourFullness=Math.max(0,yourFullness+extra.fullnessDelta);
      let extraLbs=extra.lbs||0;
      if(extra.steal){
        const avail=(mayaFoods||[]).filter(f=>!f.consumed);
        if(avail.length>0){
          const pick=avail[0];
          updates.mayaFoods=mayaFoods.map(f=>f.key===pick.key?{...f,consumed:true}:f);
          extraLbs+=pick.lbs||0;
          updates.yourFullness=Math.max(0,(updates.yourFullness??yourFullness)+Math.round((pick.fullness||0)*0.5));
        }
      }
      if(extraLbs>0){
        updates.yourGain=(yourGain||0)+extraLbs;
        setStudents(prev=>prev.map(st=>st.id===studentId?processStudentGain(st,extraLbs,0):st));
      }
      popup=s?renderContestActionPopup(action,stageIdx,s,week):'';
    } else {
      return;
    }
  ...
  const sumoCornerFeed=(feedId='default')=>{
    if(!sumoMatchState) return;
    const st=sumoMatchState;
    const s=students.find(x=>x.id===st.studentId);
    const feeds=[{id:'default',lbs:(SUMO_CORNER_FEED[st.stageIdx]||SUMO_CORNER_FEED[0]).lbs,text:(SUMO_CORNER_FEED[st.stageIdx]||SUMO_CORNER_FEED[0]).text},...extraSumoCornerFeeds(ownedHallSkills||{},st.stageIdx)];
    const feed=feeds.find(f=>f.id===feedId)||feeds[0];
    const used=st.cornerUsed||{};
    if(used[feed.id]) return;
    setStudents(prev=>prev.map(x=>x.id===st.studentId?processStudentGain(x,feed.lbs,0):x));
    setSumoMatchState({...st,gainAccum:st.gainAccum+feed.lbs,cornerUsed:{...used,[feed.id]:true},popupText:s?renderSumoCornerFeed(st.stageIdx,s,week)||feed.text:feed.text,phaseAfterPopup:'nextbout'});
  };
  ...
  const startCollabStream=(kylieId,partnerId,stageIdx,history)=>{
    ...
    const tierFoods=(collabFoodsForOwned(stageIdx,ownedHallSkills||{})).map((f,i)=>({...f,consumed:false,key:i,tierUnlocked:i===0}));
  ...
  const startFairTrainingSession=(collabKey)=>{
    ...
    const extra=extraFairTrainingLbs(ownedHallSkills||{});
    const mjGain=rnd(mjLo,mjHi)+extra, cGain=rnd(cLo,cHi)+extra;
    setStudents(prev=>prev.map(st=>{
  ...
  const startIntimacyScene=(s,sceneId)=>{
    ...
    const loungeFx=aggregateHallLoungeSkillEffects(ownedHallSkills||{});
    setIntimacyEventState({studentId:s.id,sceneId,tier,week,phaseIdx:0,history:[],logLines:[],gainAccum:0,relAccum:0,done:false,endingText:null,gainBonus:ownedHallSkills?.oversized_linens?1:0,relBonus:loungeFx.intimacyRelBonus||0});
  ...
  const makeIntimacyChoice=(choiceId)=>{
    ...
    if(nextPhase>=def.phases.length){
      ...
      const loungeRel=intimacyEventState.relBonus||0;
      const loungeGain=intimacyEventState.gainBonus||0;
      const totalGain=newGain+(ending.gainBonus||0)+loungeGain;
  ...
  const startFloorCheckIn=()=>{
    playHallPassSound('click', soundEnabled);
    const scenes=generateFloorCheckIn(students.filter(studentReceivesPassiveGain),week,ownedHallSkills||{});
  ...
  const makeChoice=(choiceIdx)=>{
    ...
    if(type==="student"&&student){
      ...
      if(s){
        gainAmt=Math.round(rnd(choice.effect.gain[0],choice.effect.gain[1])*floorCheckInGainMult(ownedHallSkills||{}));
  ...
  const completeRoomVisit=(studentId)=>{
    const visitBonus=1+(aggregateHallLoungeSkillEffects(ownedHallSkills||{}).roomVisitRelBonus||0);
    setStudents(prev=>prev.map(s=>{
      if(s.id!==studentId) return s;
      const done=applyRoomVisitComplete(s,week);
      return {...done,relationship:Math.min(100,(done.relationship||0)+visitBonus)};
    }));
    const s=students.find(x=>x.id===studentId);
  ...
  const confirmSkillPurchase=()=>{
    ...
    const updated=students.map(s=>{
      ...
      if(newStage<oldStage){
        setTimeout(()=>push(`📉 ${s.name} drops to ${WEIGHT_STAGES[newStage].label}. "${renderStageDrop({...s,lbs:newLbs}, week)||'…'}"`) ,60);
      }else{
  ...
  const resolveNarrative=(ev,s,accept)=>{
    if(accept&&ev.gain[1]>0){
      ...
      const extraRel=aggregateHallLoungeSkillEffects(ownedHallSkills||{}).weighInRelBonus||0;
      setStudents(prev=>prev.map(st=>st.id!==s.id?st:{...st,lbs:st.lbs+gain,relationship:Math.min(100,st.relationship+ev.rel+extraRel)}));
      push(`📖 ${ev.title} resolved. ${s.name} +${gain} lbs, +${ev.rel+extraRel} relationship.`);
    } else {
  ...
  const skillSessionCapBonus=(loungeSkillFx.sessionCapBonus||0)+((floorCircuit||{}).sessionCapNextWeek||0);
...
{/* ── SKILL TREE ── */}
      ...
      {/* ── ITEM TARGET PICKER ── */}
      {itemTargetPicker&&<ItemTargetPicker itemTargetPicker={itemTargetPicker} setItemTargetPicker={setItemTargetPicker} students={students} lilithUnlocked={lilithUnlocked} useItemOn={useItemOn} owned={ownedHallSkills||{}} soundEnabled={soundEnabled}/>}
      ...
      {weighInState&&<WeighInModal
        ...
        onComplete={(s)=>{
          const wr=aggregateHallLoungeSkillEffects(ownedHallSkills||{}).weighInRelBonus||0;
          if(wr>0){
            setStudents(prev=>prev.map(x=>x.id===s.id?{...x,relationship:Math.min(100,(x.relationship||0)+wr)}:x));
          }
          if((ownedSkills.memory_palace||0)>=1){
      ...
      {weekPlannerOpen&&(
        <WeekPlannerModal
          ...
          slotCount={plannerSlotCount(ownedHallSkills||{})}
          soundEnabled={soundEnabled}
            ...
            <div className="hall-pass-modal-in ranked-session-modal" style={{...C.modal,maxWidth:580,background:"linear-gradient(160deg,#040810,#080e1a,#040810)",border:"1px solid #1a5a9050",maxHeight:"90vh",overflowY:"auto",padding:20}}>
              ...
              {!done&&(
                <>
                  ...
                  {/* Food menu */}
                  <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}>
                    {sessionFoodsForOwned(ownedHallSkills||{}).map(food=>{
      ...
      {sumoMatchState&&<SumoMatchModal sumoMatchState={sumoMatchState} students={students} week={week} sumoPlayMove={sumoPlayMove} sumoCornerFeed={sumoCornerFeed} sumoStartNextBout={sumoStartNextBout} setSumoMatchState={setSumoMatchState} closeSumoMatch={closeSumoMatch} dismissSumoPopup={dismissSumoPopup} owned={ownedHallSkills||{}} soundEnabled={soundEnabled}/>}
        ...
        return(
          <CompoundFeedModal
            ...
            onConfirm={(compoundId)=>{
              ...
              if(picker.kind==='item') executeItemFeed(picker.item,picker.studentId,compoundId,picker.modeId||'feed');
