...
import { EVOLVED_ACTIVITY_TEXT, EVOLVED_ACTIVITY_META, EVOLVED_EVENTS, EVOLUTION_OFFER, HOMEROOM_SUSPICION_DELTAS, HOMEROOM_THRESHOLDS, HOMEROOM_CONFERENCE_EVENTS, HOMEROOM_GROUP_ACTIVITIES, SESSION_FOOD_ITEMS, WL_CONFIG, WL_LESSONS, WL_DIALOGUES, CG_CONFIG, CG_CORKBOARD_SCENES, CG_MEASUREMENT_SCENES, CG_BINGE_SCENES, CG_CHAT_TEMPLATES, FAIR_TRAINING_CONFIG, FAIR_DAY_SCENES } from './gameData/evolvedForms.js';
...
import {
  ...
  contestBiteLbs, depthGainLbs, evolvedPrepFx, homeroomArcFx, wifeRepeatFx,
  hiveShiftFx,
...
import './textEngine/scenes/proseOverhaulPass3.js';
...
export default function HallPass(){
  ...
  const {
    ...
    globalStats, achievements, bigScaleUnlocked, hallCred, unlockedDorms, v2State, dormState,
  } = player;
  ...
  const advanceWeek=()=>{
    ...
    let updated=students.map(s=>{
      ...
      gain=Math.max(0,Math.round(gain*oppGainMult*asceticMult*mirrorMult*getSupernaturalGainMult(nsHab)*formPassiveGainMultiplier(nsHab)*(1+(loungeSkillFx.gainMult||0))*habGain*withdrawalGainMultiplier(nsHab)));
    ...
    // ── PANTRY RESTOCK ──
    {
      const drops=rnd(INVENTORY_CONFIG.weeklyDrops[0],INVENTORY_CONFIG.weeklyDrops[1])+pantryDropBonus(ownedHallSkills||{},dormState||{});
      const found=[];
      setInventory(prev=>{
        const next={...prev};
  ...
  const doEvolvedActivity=(s)=>{
    ...
    const beat=renderEvolvedActivityBeat(s,week,stageIdx,{formId:s.evolvedForm});
    let text=beat||(rawText?(typeof rawText==='function'?rawText(s):rawText):"She's in her element.");
    ...
    let gain=depthGainLbs(s,rawGain,week,{});
    if(doubleCharge) gain*=2;
    const hab=habitatFx(s,dormState||createInitialDormState(),ownedHallSkills||{});
    if(hab.evolvedLbs) gain+=hab.evolvedLbs;
  ...
  const makeEvolvedEventChoice=(choiceId)=>{
    ...
    const choiceLbs=choice.lbs?depthGainLbs(s,choice.lbs,week,{skipNight:true}):0;
    const newGain=gainAccum+choiceLbs;
  ...
  const makeHomeroomActivityChoice=(choiceId)=>{
    ...
    const bite=choice.lbs?depthGainLbs(daisy,choice.lbs,week,{skipNight:true}):0;
  ...
  const consumeMan=()=>{
    ...
    setStudents(prev=>prev.map(s=>{
      ...
      const markTotal=Object.values(marks).reduce((a,n)=>a+(n||0),0);
      const grown=processStudentGain(s,depthGainLbs(s,gain,week,{chain:markTotal>=3?1:0}),0);
      return bumpOriginChain({...grown,huntMarks:marks});
    }));
  ...
  const deliveryScene=()=>{
    ...
    setStudents(prev=>prev.map(s=>s.id===LILITH_ID?bumpOriginChain(processStudentGain(s,depthGainLbs(s,gain,week,{}),0)):s));
    setLilithKillCount(k=>k+1);
  ...
  const runPharmacistSynthesis=(s)=>{
    ...
    guardHungerInterrupt(()=>{
      ...
      setPharmacistChemSession(startChemSession(pharmacistState,{leftover:!!s.leftoverFedThisWeek,night:s.lastNightVisitWeek===week}));
    });
  };
  ...
  const startRankedSession=(studentId,stageIdx)=>{
    ...
    const arrival=renderRankedNpcArrival(s,week,raeStage);
    setRankedFeedeeState({studentId,stageIdx,focus:maxFocus,maxFocus,fullness:0,maxFullness,gain:0,turn:0,log:[arrival],done:false,endReason:null,raeDelivered:raeStage<=1,lastFoodId:null});
    setEvolvedEventState(null);
  };
  ...
  const pickSessionFood=(foodId)=>{
    ...
    if(!raeDelivered&&newTurn===3&&stageIdx>=2){
      updatedLog=[...newLog,renderRankedNpcDrop(s,week,Math.min(5,stageIdx))];
  ...
  const startFairTrainingSession=(collabKey)=>{
    ...
    let mjGain=depthGainLbs(mj,rnd(mjLo,mjHi),week,{});
    ...
    setFairTrainingState(prev=>({...prev,
      ...
      sessionSceneTag:renderFairTrainingScene(mj,week,collabKey,cStage),
      sessionPhotoTag:renderFairTrainingPhoto(mj,week,collabKey),
      sessionBoostSummary:renderFairBoost(mj,week,collabKey,boostTier),
  ...
  const chooseFairWeighIn=(choice)=>{
    ...
    setFairDayState(prev=>{
      ...
      const baseGain=choice===1?(sc?.gainA??8):(sc?.gainB??5);
      const gain=s?depthGainLbs(s,Math.round(baseGain*(1+bonus)),week,{}):Math.round(baseGain*(1+bonus));
      const rel=choice===1?(sc?.relA??5):(sc?.relB??9);
      return {...prev,weighInChoice:choice,
        weighInResultText:s?renderFairDayWeighInResult(s,week,prev.influenceKey,choice):'',
  ...
  const makeIntimacyChoice=(choiceId)=>{
    ...
    const choiceLbs=choice.lbs?depthGainLbs(s,choice.lbs,week,{skipNight:true}):0;
    let newGain=gainAccum+choiceLbs;
    const newRel=relAccum+(choice.rel||0);
...
{/* ── SKILL TREE ── */}
      ...
      {weighInState&&<WeighInModal
        ...
        onComplete={(s)=>{
          const hab=habitatForStudent(s,dormState||createInitialDormState());
          if(hab.weighRel||hab.weighLbs){
            setStudents(prev=>prev.map(st=>{
              if(st.id!==s.id) return st;
      ...
      {rankedFeedeeState&&(()=>{
        ...
        const payoffText=done?renderRankedPayoff(s,week,stageIdx,gain,endReason):`Session closed with ${Math.round(gain)} lbs gained.`;
        const dismissRanked=()=>{
          playHallPassSound('click', soundEnabled);
      ...
      {presentationState&&<PresentationDefenseModal presentationState={presentationState} processStudentGain={processStudentGain} push={push} setPresentationState={setPresentationState} setStudents={setStudents} students={students} week={week} soundEnabled={soundEnabled}/>}
      {deliveryState&&<DeliveryOrderModal deliveryState={deliveryState} processStudentGain={processStudentGain} push={push} setDeliveryState={setDeliveryState} setStudents={setStudents} students={students} week={week} soundEnabled={soundEnabled}/>}
