...
// Phase 4  "purchase" First-ever break unlocks the heavy-duty scale.
// Phase 5  "swap"     Professor swaps in the heavy-duty scale.
// Phase 6  "digital"  Rugged digital scale that ticks up to weight.
// ═══════════════════════════════════════════════════════════════

function firstName(name){
  return (name||'her').split(' ')[0];
}
...
function getSwapScene(student){
  return (
    ...
    `digital readout. "Step up whenever you're ready, ${firstName(student.name)}."`
  );
}
...
function getPurchaseScene(student){
  return (
    ...
    `"Okay. Yeah. That's on me, not on ${firstName(student.name)}." You make a mental ` +
...
function AnalogScale({ lbs, willBreak, onBroken }){
  ...
  const readout=willBreak ? (cracked?'— ERR —':'— lbs') : (settled?`${safeLbs} lbs`:'— lbs');
...
function DigitalScale({ lbs }){
  ...
  useEffect(()=>{
    ...
    const tick=(now)=>{
      ...
      setDisplay(Math.round(eased*peak));
      if(t<1){
    ...
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:8}}>
      <div style={{
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:6,marginTop:6}}>
          <div style={segStyle}>{String(display).padStart(4,' ')}</div>
...
export function WeighInModal({ weighInState, setWeighInState, bigScaleUnlocked, brokeScaleIds=[], onBreakScale, onUnlockBigScale }){
  ...
  const willBreakNow=phase==='analog'
    ? (weighInState.scaleWillBreak??(lbs>400&&!alreadyBroke))
    : false;

  const setPhase=(nextPhase,extra={})=>setWeighInState({...weighInState,phase:nextPhase,...extra});
  ...
  const stepOntoScale=()=>{
    ...
    setPhase('analog',{scaleWillBreak:lbs>400&&!alreadyBroke});
  };
  const onAnalogBroken=()=>{
    if(onBreakScale) onBreakScale(student.id);
  };
  const handleAfterBreak=()=>{
    if(!bigScaleUnlocked){
      if(onUnlockBigScale) onUnlockBigScale();
      setPhase('purchase');
      return;
    }
    setPhase('swap');
      ...
      <div style={{...C.modal,maxWidth:560}}>
        ...
        {phase==='analog'&&(
          <>
            ...
            {willBreakNow?(
              <UhOhButton onClick={()=>setPhase('break')}/>
            ):(
              <button style={{...C.btn('#5818a8'),width:'100%'}} onClick={close}>
                Close ✓
              </button>
        ...
        {phase==='purchase'&&(
          <>
            ...
            <div style={{fontSize:11,color:'#a070d0',letterSpacing:1,marginBottom:12,textAlign:'center'}}>
