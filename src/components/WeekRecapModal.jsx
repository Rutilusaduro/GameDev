...
export function WeekRecapModal({ weekRecap, onClose }){
    ...
    <div style={C.overlay}>
      <div style={{...C.modal,maxWidth:480}}>
        ...
        <div style={{maxHeight:"60vh",overflowY:"auto",marginBottom:10,display:"flex",flexDirection:"column",gap:8}}>
          {movers.map(m=>(
            <div key={m.id} style={{...C.card,borderLeft:m.stagedUp?"3px solid #c8860a":m.stuffed?"3px solid #b04880":"3px solid #5a3888"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:6,marginBottom:6,flexWrap:"wrap"}}>
                <div style={{fontWeight:700,color:"#c090e8"}}>{m.name}</div>
