...
export function SalonAppetitModal({
  ...
  if (!session) {
    ...
    return (
      <div style={{ ...C.overlay, zIndex: 360 }}>
        <div style={{ ...C.modal, maxWidth: 560, background: '#12080c', border: `1px solid ${ACCENT}55`, maxHeight: '90vh', overflowY: 'auto' }}>
          ...
          <button type="button" style={{ ...C.btn('#444'), width: '100%', marginTop: 8 }} onClick={onClose}>Close</button>
        </div>
      </div>
    );
  }
  ...
  if (session.phase === 'menu') {
    return (
      <div style={{ ...C.overlay, zIndex: 360 }}>
        <div style={{ ...C.modal, maxWidth: 520, background: '#12080c', border: `1px solid ${GOLD}44` }}>
  ...
  if (session.phase === 'service') {
    ...
    return (
      <div style={{ ...C.overlay, zIndex: 360 }}>
        <div style={{ ...C.modal, maxWidth: 520, background: '#12080c', border: `1px solid ${ACCENT}55` }}>
  ...
  if (session.phase === 'digestif') {
    return (
      <div style={{ ...C.overlay, zIndex: 360 }}>
        <div style={{ ...C.modal, maxWidth: 480, background: '#12080c', border: `1px solid ${GOLD}66` }}>
          ...
          <button type="button" style={{ ...C.btn(GOLD), width: '100%', marginTop: 12 }} onClick={onDigestif}>Finish the evening ✓</button>
        </div>
      </div>
    );
  }
