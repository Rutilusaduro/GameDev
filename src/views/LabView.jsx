...
function BlueprintCard({ recipe, labState, taliaStudent, money, onBuild }) {
  ...
  return (
    <div style={{ ...C.card, marginBottom: 8 }}>
      ...
      {!researched && (
        <div style={{ fontSize: 9, color: '#806050', fontStyle: 'italic', marginBottom: 6 }}>
          Locked — research this blueprint in the tech tree first.
        </div>
      )}
      {researched && buildable && (
        <button
          style={{ ...C.btn(afford ? ACCENT : '#442828'), width: '100%', fontSize: 10, marginTop: 6, opacity: afford ? 1 : 0.85 }}
          disabled={!afford}
