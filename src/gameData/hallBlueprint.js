...
/** Overhaul floor rooms (percent layout). Kept for weave synergies + unused HallBlueprintView. */
...
const BLUEPRINT_ROOM_BY_ID = Object.fromEntries(HALL_BLUEPRINT_ROOMS.map((r) => [r.id, r]));
...
export function skillsForBlueprintRoom(roomId) {
  return SKILL_TREE.filter((sk) => getSkillBlueprintRoom(sk.id) === roomId);
}

export { skillsForBlueprintRoom as skillsForRoom };
...
function roomHasUpgrade(roomId, owned = {}) {
  return skillsForBlueprintRoom(roomId).some((sk) => owned[sk.id]);
}
...
export function getRoomDisplayMeta(roomId, ownedHallSkills = {}) {
  const room = BLUEPRINT_ROOM_BY_ID[roomId];
  if (!room) return null;
  const skills = skillsForBlueprintRoom(roomId);
...
export function summarizeHallEnvironment(ownedHallSkills = {}) {
  ...
  const sanctumOwned = skillsForBlueprintRoom('sanctum').filter((sk) => ownedHallSkills[sk.id]).length;
  const kitchenOwned = skillsForBlueprintRoom('kitchen').filter((sk) => ownedHallSkills[sk.id]).length;
  return {
