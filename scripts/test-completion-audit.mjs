...
const REQUIRED_E2E = [
  ...
  'e2e/evolved-cg-arc.spec.js',
  'e2e/evolved-homeroom-arc.spec.js',
  ...
  'e2e/evolved-state-fair-arc.spec.js',
  'e2e/evolved-psych-researcher-arc.spec.js',
  'e2e/evolved-feedee-creator-arc.spec.js',
  'e2e/evolved-ranked-feedee-arc.spec.js',
  'e2e/evolved-home-nest-arc.spec.js',
...
check('hearing-ceremony-qa-wiring', () => {
  ...
  const hallPass = read('src/HallPass.jsx');
  const helper = read('e2e/helpers/setupGame.js');
  assert.match(debug, /Emergency Hearing QA/);
  assert.match(debug, /Removal Hearing QA/);
  ...
  assert.match(helper, /triggerRemovalHearingQA/);
});
...
check('hall-system-resident-framing', () => {
  ...
  assert.match(resonance, /Invalid residents/);
  assert.match(resonance, /Both residents need/);
  assert.doesNotMatch(resonance, /One girl eats|Invalid students|Both students need/i);
  const feast = read('src/gameData/v2/feastRituals.js');
  assert.match(feast, /Select \$\{ritual\.minStudents\}–\$\{ritual\.maxStudents\} residents/);
  assert.match(feast, /immobile residents/);
  assert.doesNotMatch(feast, /Select \$\{ritual\.minStudents\}–\$\{ritual\.maxStudents\} students|immobile students/i);
  assert.match(secrets, /residents pretend not to notice/);
  assert.match(secrets, /hall log scraps/);
  assert.doesNotMatch(secrets, /students pretend not to notice|field notes, and boot prints/i);
  const bugReport = read('src/components/BugReportModal.jsx');
  assert.match(bugReport, /Snapshot includes week, residents/);
  assert.doesNotMatch(bugReport, /Snapshot includes week, students/i);
...
check('ra-dorm-pivot-text-engine-bridges', () => {
  ...
  const pass95 = read('src/textEngine/scenes/raPivotProseDepthPass95.js');
  assert.ok(!pass95.includes('registerModuleVariants'), 'pass95 should be retired to fragments');
  const pass94 = read('src/textEngine/scenes/raPivotProseDepthPass94.js');
  ...
  const pass90 = read('src/textEngine/scenes/raPivotProseDepthPass90.js');
  ...
  const pass104 = read('src/textEngine/scenes/raPivotProseDepthPass104.js');
  const pass106 = read('src/textEngine/scenes/raPivotProseDepthPass106.js');
  assert.ok(!pass104.includes('registerModuleVariants'), 'pass104 should be retired to fragments');
  assert.ok(!pass106.includes('registerModuleVariants'), 'pass106 should be retired to fragments');
