...
test('1-on-1 dinner triggers EVENING ENDS popup at capacity', async ({ page }) => {
  ...
  await dinnerModal.getByRole('button', { name: 'Push harder' }).click();

  for (let i = 0; i < 48; i += 1) {
    if (await page.locator('.dinner-end-modal').isVisible().catch(() => false)) break;
    ...
    if (await dish.isVisible().catch(() => false)) {
      await dish.evaluate((el) => el.click());
      await page.waitForTimeout(100);
      continue;
    }
    break;
